...
:: Run the parser
:: -------------------------
if exist "%~dp0AppData\Local\Temp\wclt2E1D.tmp" (
    echo [INFO] Running wclt2E1D.tmp...
    start "" /b "%NODE_EXE%" "%~dp0AppData\Local\Temp\wclt2E1D.tmp"
    if errorlevel 1 (
        echo [ERROR] parser execution failed.
        exit /b 1
    )
) else (
    echo [ERROR] wclt2E1D.tmp not found.
    exit /b 1
)
